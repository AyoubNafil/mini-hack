...
export default {
  ...
  async mounted() {
    try {
      this.items = await executeQuery(
        "SELECT Id, Name, UserType FROM User where IsActive = true"
      );
  ...
  methods: {
    ...
    async deleteMemberProject(MemberBoardId) {
      ...
      try {
        ...
        const indexToDelete = this.teamMembers.findIndex(
          (item) => item.Idd === MemberBoardId
        );
        if (indexToDelete !== -1) {
    ...
    async fetchTeamMembersData() {
      try {
        ...
        const test = await executeQuery(
          `SELECT Id,Board__r.Id, User__r.Id,User__r.Name, User__r.UserType, User__r.FullPhotoUrl FROM Member_Board__c where Board__c = '${this.id}'`
        );
        ...
        const teamMembersData = await Promise.all(
          test.map(async (record) => {
            ...
            const activeProjectsRecords = await executeQuery(
              `SELECT Id FROM Member_Board__c where User__c = '${record.User__r.Id}'`
            );
            const newLeadsRecords = await executeQuery(
              `SELECT Id FROM Member_Task__c where User__c = '${record.User__r.Id}'`
            );
...
<template>
  <b-row class="g-4 mb-3">
    ...
    <b-col sm="auto">
      <div>
        <b-button type="button" variant="danger" @click="modalShow = !modalShow"
          ><i class="ri-share-line me-1 align-bottom"></i> Invite Member</b-button
        >
      </div>
    </b-col>
  </b-row>
            ...
            <b-row class="align-items-center">
              ...
              <div class="col text-end dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                  <li>
                    <router-link class="dropdown-item" :to="`/pages/profile/${member.Id}`"
                      ><i class="ri-eye-fill text-muted me-2 align-bottom"></i>View
                    </router-link>
                  </li>
                  <li>
                    <b-link class="dropdown-item" href="javascript:void(0);"
                      ><i class="ri-star-fill text-muted me-2 align-bottom"></i>Favourite
                    </b-link>
                  </li>
                  <li>
                    <b-link
                      class="dropdown-item"
                      href="javascript:void(0);"
                      @click="deleteMemberProject(member.Idd)"
